...
const ChatAssistant = ({ open, onOpenChange }: ChatAssistantProps) => {
  ...
  const savePreferences = async () => {
    ...
    const { error } = await supabase.from("user_preferences").upsert(
      {
        user_id: user.id,
        favorite_categories: pickedCats,
        budget_preference: pickedBudget || null,
        online_vs_instore: pickedMode || null,
        onboarded: true,
      },
      { onConflict: "user_id" },
    );
  ...
  const send = async (text?: string) => {
    ...
    try {
      ...
      if (!resp.ok || !resp.body) {
        if (resp.status === 429) toast.error("A bit busy — try again in a moment.");
        else if (resp.status === 402) toast.error("AI credits exhausted. Please contact support.");
        else toast.error("Couldn't reach the assistant.");
        setLoading(false);
        return;
      }
