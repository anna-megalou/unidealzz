...
export const ExperienceComposer = ({ open, onOpenChange, onPosted }: Props) => {
  ...
  const handleSubmit = async () => {
    ...
    setSubmitting(true);
    try {
      const savingsNum = savings.trim() ? Number(savings.replace(",", ".")) : null;
      ...
      const { data: post, error: postErr } = await supabase
        ...
        .insert({
          ...
          rating: parsed.data.rating > 0 ? parsed.data.rating : null,
          would_recommend: parsed.data.wouldRecommend,
          savings_amount: Number.isFinite(savingsNum as number) ? savingsNum : null,
        ...
        const uploads = await Promise.all(
          files.map(async (file, i) => {
            ...
            const { data: pub } = supabase.storage
              .from("experience-images")
              .getPublicUrl(path);
            return { post_id: post.id, user_id: user.id, image_url: pub.publicUrl, position: i };
          }),
        );
      ...
      toast.error(e?.message ?? "Could not publish your post");
    } finally {
      setSubmitting(false);
    }
  };
