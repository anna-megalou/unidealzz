  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const { data: offers } = await supabase
        .from("offers")
        .select("id,title,active,featured,created_at,brand:brands(name),category:categories(name)")
        .order("created_at", { ascending: false })
        .limit(50);

      const offerIds = (offers ?? []).map((o: any) => o.id);
      let claimsByOffer: Record<string, number> = {};
      if (offerIds.length) {
        ...
        (claims ?? []).forEach((c: any) => {
          claimsByOffer[c.offer_id] = (claimsByOffer[c.offer_id] ?? 0) + 1;
        });
      }
