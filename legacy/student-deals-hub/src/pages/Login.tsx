...
const Login = () => {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    if (error) {
      setSubmitting(false);
      if (error.message.toLowerCase().includes("invalid")) toast.error(t("login.error.invalid"));
      else if (error.message.toLowerCase().includes("not confirmed")) toast.error(t("login.error.notConfirmed"));
  ...
  const handleForgotPassword = async () => {
    ...
    else toast.success(t("login.forgot.sent"));
  };
