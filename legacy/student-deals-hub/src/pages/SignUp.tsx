...
const SignUp = () => {
  ...
  const handleSignUp = async (e: React.FormEvent) => {
    ...
    // Submit student verification immediately at signup.
    // RPC auto-approves if email domain (or subdomain) matches the university's allowed domains.
    if (universityId && signUpData.session) {
      try {
        await supabase.rpc("submit_student_verification", {
          _university_id: universityId,
          _student_email: email,
        });
      } catch {
        // Non-blocking — can be retried later.
      }
    }
