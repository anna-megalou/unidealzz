...
import { X, Backpack, Plane, Film } from "lucide-react";
  ...
  const fetchSaved = async () => {
    ...
    const { data, error } = await supabase
      ...
      .select(
        `id, created_at,
         offer:offers(
           id, title, description, image_url, discount_label, featured,
           brand:brands(name),
    ...
    const rows: SavedRow[] = (data ?? [])
      ...
      .map((r: any) => ({
        ...
        category: (r.offer.category?.name ?? r.offer.brand?.name ?? "OFFER").toUpperCase(),
        badge: r.offer.discount_label ?? (r.offer.featured ? "FEATURED" : "SAVED"),
      }));
      ...
      <div className="-mx-8 -my-8 min-h-[calc(100vh-4rem)] bg-gradient-to-b from-[hsl(280_60%_98%)] to-[hsl(280_60%_98%)] px-8 py-10">
        ...
        <section className="mt-16">
          <h2 className="mb-6 font-display text-3xl font-bold text-foreground">
            Recommended for{" "}
            <span className="italic text-emerald-600">You</span>
          </h2>
