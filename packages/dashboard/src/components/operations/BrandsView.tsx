...
import { archiveBrand, restoreBrand } from "@/lib/operationsApi";
...
const BrandsView = forwardRef<BrandsViewHandle>((_, ref) => {
  ...
  const loadArchived = useCallback(async () => {
    ...
    const data = await fetchBrands(false);
    setArchivedBrands(data.filter((b) => b.archived_at) as BrandRow[]);
  ...
  const handleRestore = async (b: BrandRow) => {
    ...
    try {
      ...
      toast.error(err instanceof Error ? err.message : "Could not restore brand");
    } finally {
      setRestoringId(null);
    }
  };
  ...
  const handleArchive = async () => {
    ...
    setArchiveBusy(true);
    try {
      await archiveBrand(archiving.id);
      ...
      toast.error(err instanceof Error ? err.message : "Could not archive brand");
    } finally {
