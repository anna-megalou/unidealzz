...
export { app, db, auth, functions, storage, appCheck, messaging, analytics, googleProvider, USE_EMULATORS };

export { Collections } from './collections';

// Callable functions
import type { UserAccountProfile } from '@unidealz/shared';
...
export const callAcceptTeamInvite = httpsCallable<Record<string, never>, { success: boolean; error?: string }>(
  functions,
  'acceptTeamInvite',
);

export const callChatAssistant = httpsCallable<
  { messages: { role: string; content: string }[]; context?: Record<string, unknown> },
  { reply: string }
>(functions, 'chatAssistant');

export const callManageBrand = httpsCallable<
  { action: 'create' | 'update' | 'archive' | 'updateUser'; brandId?: string; payload: Record<string, unknown> },
  { brandId: string }
>(functions, 'manageBrand');

export const callSendTeamInvite = httpsCallable<
  { email: string; role: string },
  { success: boolean }
>(functions, 'sendTeamInvite');

export const callDeleteStaffMember = httpsCallable<
  { userId: string },
  { success: boolean }
>(functions, 'deleteStaffMember');

export const callSignUpStudent = httpsCallable<
  {
    firstName: string;
    lastName: string;
    email: string;
  ...
  { success: boolean; requiresVerification?: boolean }
>(functions, 'signUpStudent');

export const callSubmitStudentVerification = httpsCallable<
  { universityId: string; studentEmail: string },
  { success: boolean }
...
export const callSendTransactionalEmail = httpsCallable<
  {
    templateName: string;
  ...
  { success: boolean }
>(functions, 'sendTransactionalEmail');
