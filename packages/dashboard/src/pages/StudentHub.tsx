...
import { fetchHubSummary, fetchPurchasesWithProfile } from "@/lib/studentHubApi";
import { fetchActiveOffers, fetchUserClaimedOffers, fetchUserFavorites } from "@/lib/firestoreData";
...
const StudentHub = () => {
  ...
  useEffect(() => {
    ...
    (async () => {
      const summary = await fetchHubSummary(user.uid);
      if (cancelled) return;

      setAvatarUrl(summary.avatarUrl);
      setDisplayName(summary.displayName || student.firstName);
      setVerification({
        status: summary.verification.status,
        verifiedAt: summary.verification.verifiedAt,
      });
      setClaimedCount(summary.claimedCount);
      setSavedCount(summary.savedCount);

      const [claimed, favorites, offers] = await Promise.all([
        fetchUserClaimedOffers(user.uid),
      ...
      if (summary.purchaseCount > 0) {
        const { purchases: purchaseRows } = await fetchPurchasesWithProfile(user.uid);
        const real = purchaseRows.reduce((sum, p) => sum + Number(p.amount_saved ?? 0), 0);
        setLifetimeSavings(real);
      } else {
        const ESTIMATED_AVG_PRICE = 50;
        const savings = await Promise.all(
          claimed.map(async (c) => {
            const offer = c.offer_id ? offers.find((o) => o.id === c.offer_id) : null;
            const pct = offer?.discount_percent ?? 15;
            return ESTIMATED_AVG_PRICE * (pct / 100);
          }),
        );
        setLifetimeSavings(savings.reduce((a, b) => a + b, 0));
      }

      const acts: ActivityItem[] = [];
      for (const c of claimed.slice(0, 3)) {
        const offer = c.offer_id ? offers.find((o) => o.id === c.offer_id) : null;
        acts.push({
          id: `c-${c.id}`,
          icon: Ticket,
          title: t("hub.activity.claimed").replace("{title}", offer?.title ?? "Offer"),
          meta: `${formatRelative(c.claimed_at)} • ${offer?.brand?.name ?? "Brand"}`,
          tone: "primary",
        });
      }
      for (const f of favorites.slice(0, 3)) {
        const offerId = String(f.offerId ?? f.id);
        const offer = offers.find((o) => o.id === offerId);
        acts.push({
          id: `f-${f.id}`,
          icon: Heart,
          title: t("hub.activity.saved").replace("{title}", offer?.title ?? "Offer"),
          meta: `${formatRelative(String(f.createdAt ?? ""))} • ${offer?.category?.name ?? "Saved"}`,
          tone: "fashion",
        });
      }
      if (summary.verification.verifiedAt && summary.verification.status === "approved") {
        acts.push({
          id: "v-approved",
          icon: ShieldCheck,
          title: t("hub.activity.verified"),
          meta: `${formatRelative(summary.verification.verifiedAt)} • System`,
          tone: "success",
        });
      }
      ...
      const preferredCategoryIds = [
        ...new Set(
          [
            ...favorites.map((f) => {
              const offerId = String(f.offerId ?? f.id);
              return offers.find((o) => o.id === offerId)?.category?.id;
            }),
            ...claimed.map((c) => offers.find((o) => o.id === c.offer_id)?.category?.id),
          ].filter(Boolean),
        ),
      ] as string[];

      let offerRows = offers.filter(
        (o) => preferredCategoryIds.length === 0 || preferredCategoryIds.includes(o.category?.id ?? ""),
      );
      if (offerRows.length < 2) {
        offerRows = [...offers].sort((a, b) => Number(b.featured) - Number(a.featured));
      }

      const mapped: RecommendedOffer[] = offerRows.slice(0, 2).map((o) => ({
        id: o.id,
