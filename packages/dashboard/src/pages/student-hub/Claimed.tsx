...
import { fetchClaimedDeals } from "@/lib/studentHubApi";
import { fetchOfferById } from "@/lib/firestoreData";
...
const Claimed = () => {
  ...
  useEffect(() => {
    ...
    (async () => {
      setLoading(true);
      try {
        const claimed = await fetchClaimedDeals(user.uid);
        const mapped = await Promise.all(
          claimed.map(async (c) => {
            const offer = c.offerId ? await fetchOfferById(c.offerId) : null;
            return {
              id: c.id,
              ...
              brand: c.brand ?? offer?.brand?.name ?? "Brand",
              category: (offer?.category?.name ?? "Offer").toUpperCase(),
              discountLabel: offer?.discount_label ?? null,
    ...
    <StudentHubLayout>
      <header className="mb-8 flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
        <div>
          ...
          <p className="mt-3 max-w-xl text-sm text-muted-foreground">
            Your full redemption history. Re-open codes or revisit brand pages anytime.
          </p>
        </div>
      ...
      {loading ? (
        ...
        <section className="rounded-3xl bg-card p-10 text-center ring-1 ring-border">
          ...
          <h2 className="mt-4 font-display text-2xl font-bold text-foreground">No claims yet</h2>
          <p className="mt-2 text-sm text-muted-foreground">
            Once you claim an offer, it'll show up here so you can find your codes again.
          </p>
