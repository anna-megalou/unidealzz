...
const Purchases = () => {
  ...
  const fetchAll = async () => {
    ...
    try {
      const { purchases, displayName: name, settings } = await fetchPurchasesWithProfile(user.uid);
      setRows(purchases as PurchaseRow[]);
      setDisplayName(
        name ||
          (user.email ? user.email.split("@")[0] : "there"),
      );
      const savingsGoal = settings?.savingsGoal ?? settings?.savings_goal;
      if (savingsGoal != null) {
        const g = Number(savingsGoal);
        setGoal(g);
  ...
  const totals = useMemo(() => {
    ...
    for (const [name, amount] of merchantTotals.entries()) {
  ...
  const topMerchant = totals.topMerchant;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await addPurchase(user.uid, {
        ...
        amountPaid: paidNum,
        amountSaved: isNaN(savedNum) ? 0 : savedNum,
        currency: "EUR",
        paymentMethod,
        note: note.trim() || null,
        source: "manual",
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      await deletePurchase(user.uid, id);
  ...
  const handleSaveGoal = async () => {
    ...
    try {
      ...
      await updateUserSettings(user.uid, { ...existing, savingsGoal: num });
      setGoal(num);
        ...
        <aside className="space-y-6">
                ...
                <div className="mt-2 flex items-center gap-3">
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="text-[11px] opacity-80">
                      Saved {formatMoney(topMerchant.amount)}
                    </p>
                  </div>
                </div>
              </div>
          ...
          <div className="rounded-3xl bg-muted/60 p-6">
            ...
            <Button
              ...
              title="Coming soon"
            >
              <BadgeCheck size={16} /> Connect Student ID
            </Button>
