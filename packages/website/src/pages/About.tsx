...
import { libraryImg, laptopImg, studentsImg } from '@/lib/assets';
import { usePublicStats } from '@/hooks/queries';
import { callSubmitPartnershipRequest } from '@/lib/firebase';
...
const About = () => {
  ...
  const { data: publicStats, isLoading: verifiedLoading } = usePublicStats();
  const verifiedCount = publicStats?.registeredStudentsCount ?? 0;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSubmitting(true);
    try {
      await callSubmitPartnershipRequest({
        email: email.trim(),
      ...
      toast.error(t('about.partners.form.error'), {
              ...
              <div className="space-y-4">
                ...
                <div className="rounded-2xl bg-primary p-6 text-primary-foreground">
                  <p className="font-display text-3xl font-extrabold">
                    {verifiedLoading ? '...' : formatVerified(verifiedCount)}
                  </p>
